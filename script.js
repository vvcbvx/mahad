const UPDATES_BIN_ID = '68a6ed87d0ea881f405f183b';
let currentUpdates = [];
let hasVoted = JSON.parse(localStorage.getItem('hasVoted')) || {};
const SCHEDULES_BIN_ID = '688e69e27b4b8670d8abcacc'; // استبدل بمعرف الـ Bin الخاص بك
let currentSchedules = [];


// استبدل هذه المتغيرات في بداية الملف:
const EXAMS_BIN_ID = '688323717b4b8670d8a70100'; // للامتحانات العادية
const QUIZZES_BIN_ID = '6891f6f0ae596e708fc21e60'; // للامتحانات السريعة
const JSONBIN_API_KEY = '$2a$10$3bvwkFEhQyUebdIpUA0VT.pATBpC.x.wfgo3qFQ/e2K5cu4gdCSAG';

let currentScheduleGrade = 't9';
...
window.addEventListener('load', () => {
    ...
    document.getElementById('themeToggle').addEventListener('click', toggleDarkMode);
        ...
        messageForm.addEventListener('submit', async function(e) {
            ...
            if (!name || !message) {
               showNotification('الرجاء تعبئة جميع الحقول', 'warning', 3000);
...
const chatId = '6068899411';
            ...
            try {
                ...
                if (result.ok) {
                    ...
                    showNotification('تم إرسال رسالتك بنجاح!', 'success', 3000);
...
// متغيرات الجداول الدراسية


// تحميل الجداول الدراسية من JSONBin.io
async function loadSchedules() {
    const container = document.getElementById('scheduleContainer');
    const loader = document.getElementById('schedulesLoader');
    
    loader.style.display = 'flex';
    container.innerHTML = '';
    
    try {
        const response = await fetch(`https://api.jsonbin.io/v3/b/${SCHEDULES_BIN_ID}/latest`, {
            headers: {
                'X-Master-Key': '$2a$10$3bvwkFEhQyUebdIpUA0VT.pATBpC.x.wfgo3qFQ/e2K5cu4gdCSAG',
                'Content-Type': 'application/json'
            }
        });
        ...
        const data = await response.json();
        currentSchedules = data.record.schedules;
        ...
        container.innerHTML = `
            <div class="error-message">
                <i class="fas fa-exclamation-triangle"></i>
                <p>حدث خطأ في تحميل الجداول الدراسية. يرجى المحاولة لاحقاً</p>
                <button onclick="loadSchedules()" class="retry-btn">إعادة المحاولة</button>
            </div>
        `;
    } finally {
        loader.style.display = 'none';
...
function renderSchedules() {
    ...
    // تصفية الجداول حسب الصف المحدد
    const filteredSchedules = currentSchedules.filter(s => s.grade === currentScheduleGrade);
    
    if (filteredSchedules.length === 0) {
        container.innerHTML = `
            <div class="no-schedules">
                <i class="fas fa-calendar-times"></i>
                <p>لا توجد جداول متاحة لهذه المرحلة حالياً</p>
            </div>
        `;
        return;
    }
    
    // عرض الجدول الأول (يمكن تطويرها لعرض عدة جداول)
    const schedule = filteredSchedules[0];
    const scheduleElement = document.createElement('div');
    scheduleElement.className = 'schedule-table';
    ...
    scheduleElement.innerHTML = tableHTML;
        ...
        mobileHTML += `
            <div class="mobile-day">
                ...
                <div class="day-periods">
                    <div class="period">
                        <span class="period-number">الحصة الأولى:</span>
                        <span class="period-subject">${day.periods[0] || '-'}</span>
                    </div>
                    <div class="period">
                        <span class="period-number">الحصة الثانية:</span>
                        <span class="period-subject">${day.periods[1] || '-'}</span>
                    </div>
                    <div class="period">
                        <span class="period-number">الحصة الثالثة:</span>
                        <span class="period-subject">${day.periods[2] || '-'}</span>
                    </div>
                    <div class="period">
                        <span class="period-number">الحصة الرابعة:</span>
                        <span class="period-subject">${day.periods[3] || '-'}</span>
                    </div>
                </div>
            </div>
        `;
    ...
    mobileSchedule.innerHTML = mobileHTML;
    container.appendChild(mobileSchedule);
}
...
function setupScheduleFilters() {
    document.querySelectorAll('.grade-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            this.classList.add('active');
            currentScheduleGrade = this.dataset.grade;
            renderSchedules();
        });
    });
}
...
function hideAllSections() {
    const sections = [
        ...
        'branchDetails', 'about', 'contact', 
        'developer', 'teachers', 'exams',
        'quickExams', 'schedules', 'zr', 'showFees'
    ];
...
function setupScheduleFilters() {
    // تحديد زر البكالوريا افتراضياً
    document.querySelector('.grade-btn[data-grade="bac"]').classList.add('active');
...
function showSchedules() {
    hideAllSections();
    document.getElementById('schedules').style.display = 'block';
    currentScheduleGrade = 'bac'; // تعيين البكالوريا كقيمة افتراضية
    loadSchedules();
    window.scrollTo({ top: 0, behavior: 'smooth' });
    closeSidebar();
}
...
function setupScheduleFilters() {
    // تفعيل زر البكالوريا افتراضيًا
    const bacBtn = document.querySelector('.grade-btn[data-grade="bac"]');
    if (bacBtn) {
...
// متغيرات نظام التحديثات


// دالة جلب التحديثات من JSONBin.io
async function loadUpdates() {
    try {
        const response = await fetch(`https://api.jsonbin.io/v3/b/${UPDATES_BIN_ID}/latest`, {
            headers: {
                'X-Master-Key': '$2a$10$3bvwkFEhQyUebdIpUA0VT.pATBpC.x.wfgo3qFQ/e2K5cu4gdCSAG',
                'Content-Type': 'application/json'
            }
        ...
        const data = await response.json();
        currentUpdates = data.record.updates || [];
        
        // تصفية التحديثات النشطة فقط (غير المنتهية)
        const activeUpdates = currentUpdates.filter(update => {
            if (!update.expiryTime) return true; // التحديثات بدون وقت انتهاء تعتبر نشطة
            return update.expiryTime > Date.now();
        });
        ...
        document.getElementById('updatesContainer').innerHTML = `
            <div class="error-message">
                <i class="fas fa-exclamation-triangle"></i>
                <p>حدث خطأ في تحميل التحديثات</p>
            </div>
        `;
    }
}
...
function renderUpdates(updates) {
    ...
    updates.forEach(update => {
        ...
        if (update.image) {
            updateHTML += `<img src="${update.image}" alt="${update.title}" class="update-image">`;
        }
        
        updateHTML += `</div>`;
        
        // إضافة نظام التصويت إذا كان موجوداً
        if (update.hasPoll) {
            const hasUserVoted = hasVoted[update.id];
            const totalVotes = update.yesVotes + update.noVotes;
            const yesPercentage = totalVotes > 0 ? Math.round((update.yesVotes / totalVotes) * 100) : 0;
            const noPercentage = totalVotes > 0 ? Math.round((update.noVotes / totalVotes) * 100) : 0;
            
            updateHTML += `
                <div class="update-poll">
                    <div class="poll-question">ما رأيك في هذا التحديث؟</div>
                    <div class="poll-buttons ${hasUserVoted ? 'disabled' : ''}">
                        <button class="poll-btn yes-btn" onclick="vote('${update.id}', 'yes')" ${hasUserVoted ? 'disabled' : ''}>
                            <i class="fas fa-thumbs-up"></i> نعم
                        </button>
                        <button class="poll-btn no-btn" onclick="vote('${update.id}', 'no')" ${hasUserVoted ? 'disabled' : ''}>
                    ...
                    ${hasUserVoted ? `
                        <div class="poll-results">
                            <div class="result-bar">
                                <div class="result-fill yes-fill" style="width: ${yesPercentage}%"></div>
                                <span class="result-text">نعم: ${yesPercentage}% (${update.yesVotes})</span>
                            </div>
                            <div class="result-bar">
                                <div class="result-fill no-fill" style="width: ${noPercentage}%"></div>
                                <span class="result-text">لا: ${noPercentage}% (${update.noVotes})</span>
                            </div>
                        </div>
                    ` : ''}
                </div>
...
// دالة التصويت
async function vote(updateId, voteType) {
    // التحقق إذا كان المستخدم قد صوت مسبقاً
    if (hasVoted[updateId]) {
       showNotification('لقد قمت بالتصويت مسبقاً على هذا التحديث', 'warning', 3000);
        return;
    }
    ...
    try {
        ...
        if (updateIndex !== -1) {
            if (voteType === 'yes') {
                currentUpdates[updateIndex].yesVotes += 1;
            } else {
                currentUpdates[updateIndex].noVotes += 1;
            }
            
            // تحديث حالة التصويت للمستخدم
            hasVoted[updateId] = true;
            localStorage.setItem('hasVoted', JSON.stringify(hasVoted));
            
            // تحديث الواجهة
            renderUpdates(currentUpdates.filter(update => {
                if (!update.expiryTime) return true;
                return update.expiryTime > Date.now();
            }));
            
            // محاولة تحديث البيانات على الخادم (اختياري)
            try {
                await fetch(`https://api.jsonbin.io/v3/b/${UPDATES_BIN_ID}`, {
                    method: 'PUT',
                    headers: {
                        'X-Master-Key': '$2a$10$3bvwkFEhQyUebdIpUA0VT.pATBpC.x.wfgo3qFQ/e2K5cu4gdCSAG',
                        'Content-Type': 'application/json'
                    },
                    body: JSON.stringify({ updates: currentUpdates })
                });
            } catch (error) {
                console.error('Error updating votes on server:', error);
                // يمكن إضافة رسالة للمستخدم أن التصويت تم حفظه محلياً فقط
            }
            
           showNotification('شكراً لك على التصويت!', 'success', 3000);
        }
    } catch (error) {
        console.error('Error voting:', error);
        alert('حدث خطأ أثناء التصويت، يرجى المحاولة لاحقاً');
    }
}

// في دالة showHome، إضافة تحميل التحديثات
function showHome() {
    hideAllSections();
    document.getElementById('home').style.display = 'flex';
    loadUpdates(); // تحميل التحديثات
    window.scrollTo({ top: 0, behavior: 'smooth' });
    closeSidebar();
    document.getElementById("qou").style.display="block";
}
...
// دالة تبديل عرض التحديثات
function toggleUpdates() {
    const updatesSection = document.getElementById('updatesSection');
    const isVisible = updatesSection.style.display === 'block';
    
    if (isVisible) {
        updatesSection.style.display = 'none';
    } else {
        updatesSection.style.display = 'block';
        // تحميل التحديثات إذا لم تكن محملة
        if (currentUpdates.length === 0) {
            loadUpdates();
        }
        // التمرير إلى قسم التحديثات
        updatesSection.scrollIntoView({ behavior: 'smooth' });
    }
}

// تعديل دالة renderUpdates لتعمل مع التصميم الجديد
function renderUpdates(updates) {
    const container = document.getElementById('updatesContainer');
    const updatesCount = document.getElementById('updatesCount');
    
    // تحديث عدد التحديثات في البادج
...
// تعديل دالة showHome لإضافة زر التحديثات
function showHome() {
    hideAllSections();
    document.getElementById('home').style.display = 'flex';
    loadUpdates(); // تحميل التحديثات
...
function renderAllStudents(students) {
    ...
    students.forEach(student => {
        ...
        row.innerHTML = `
            ...
            <td class="desktop-only">${student.average || 0}%</td>
            <td class="desktop-only">${student.attendance || 0}%</td>
            <td class="desktop-only">${student.achievements || '-'}</td>
        `;
    ...
    document.querySelectorAll('.exams-filter .filter-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            if (currentFilter === 'all') {
                ...
                subjectFilter.style.display = 'flex';
                // إعادة تعيين فلتر المواد إلى "الكل" عند تغيير فلتر الصف
                document.querySelector('.subject-filter .active')?.classList.remove('active');
                document.querySelector('.subject-filter [data-filter="all"]').classList.add('active');
                currentSubjectFilter = 'all';
            }
...
async function loadExams() {
    ...
    try {
        const response = await fetch(`https://api.jsonbin.io/v3/b/${EXAMS_BIN_ID}/latest`, {
            headers: {
                'X-Master-Key': JSONBIN_API_KEY,
                'Content-Type': 'application/json'
            }
...
async function downloadExam(id, fileName) {
    ...
    try {
        ...
        // تحديث البيانات على الخادم - استخدام المعرف الصحيح للامتحانات العادية
        const response = await fetch(`https://api.jsonbin.io/v3/b/${EXAMS_BIN_ID}`, {
            method: 'PUT',
            headers: {
                'X-Master-Key': JSONBIN_API_KEY,
                'Content-Type': 'application/json'
        ...
        console.error('Error downloading exam:', error);
        showNotification('حدث خطأ أثناء تحميل الامتحان. يرجى المحاولة لاحقاً', 'error', 3000);
    }
}
