// dashboard.js - لوحة التحكم لمعهد بالعلم نرتقي

// API Keys لـ JSONBin.io
const JSONBIN_API_KEYS = {
    USERS: {
        BIN_ID: '68832bebae596e708fbb6e54',
        API_KEY: '$2a$10$3bvwkFEhQyUebdIpUA0VT.pATBpC.x.wfgo3qFQ/e2K5cu4gdCSAG'
    },
    EXAMS: {
        BIN_ID: '688323717b4b8670d8a70100',
        API_KEY: '$2a$10$3bvwkFEhQyUebdIpUA0VT.pATBpC.x.wfgo3qFQ/e2K5cu4gdCSAG'
    },
    TEACHERS: {
        BIN_ID: '6883234cae596e708fbb6a7b',
        API_KEY: '$2a$10$3bvwkFEhQyUebdIpUA0VT.pATBpC.x.wfgo3qFQ/e2K5cu4gdCSAG'
    },
    BRANCHES: {
        BIN_ID: '688329e8f7e7a370d1eda081',
        API_KEY: '$2a$10$3bvwkFEhQyUebdIpUA0VT.pATBpC.x.wfgo3qFQ/e2K5cu4gdCSAG'
    },
    STUDENTS: {
        BIN_ID: '688322e77b4b8670d8a700c5',
        API_KEY: '$2a$10$3bvwkFEhQyUebdIpUA0VT.pATBpC.x.wfgo3qFQ/e2K5cu4gdCSAG'
    }
};
...
// معالجة حفظ الامتحان (إضافة/تعديل)
        ...
        if (editMode && currentEditId) {
            ...
            if (examIndex !== -1) {
                ...
                if (fileInput.files[0]) {
                    ...
                    exams[examIndex].content = base64File;
                    exams[examIndex].file = `exam_${Date.now()}.pdf`;
                }
            ...
            // إنشاء كائن الامتحان الجديد
            const newExam = {
                id: generateId(),
                title: title,
                subject: subject,
                grade: grade,
                file: `exam_${Date.now()}.pdf`,
        ...
        contentElement.innerHTML = `
            <div class="manage-branches">
                <div class="section-header">
                    ...
                    <p>يمكنك من هنا إضافة وتعديل وحذف الشعب الدراسية</p>
                    ${createBackButtons()}
                </div>
    ...
    try {
        ...
        if (editMode && currentEditId) {
            ...
            if (examIndex !== -1) {
                ...
                // إذا تم تحميل ملف جديد، احفظ اسم الملف فقط
                if (fileInput.files[0]) {
                    const file = fileInput.files[0];
                    exams[examIndex].file = `exam_${Date.now()}_${file.name}`;
                    exams[examIndex].content = ''; // لا نحتاج إلى تخزين المحتوى
                }
            ...
            const newExam = {
                ...
                subject: subject,
                grade: grade,
                file: fileName, // تخزين اسم الملف فقط
                publishDate: new Date().toISOString(),
                publisher: currentUser.name || currentUser.username,
                downloads: 0,
                content: '' // لا نحتاج إلى تخزين المحتوى
            };
        ...
        if (editMode && currentEditId) {
            ...
            if (examIndex !== -1) {
                ...
                if (fileInput.files[0]) {
                    ...
                    exams[examIndex].file = file.name;
...
async function addDistinguishedStatus(studentId, branchId) {
    try {
        ...
        // التحقق من عدد الطلاب المتميزين في الشعبة
        const distinguishedCount = students.filter(s => 
            s.branchId === branchId && s.isDistinguished
        ).length;
        
        if (distinguishedCount >= 3) {
            showMessage('لا يمكن إضافة أكثر من 3 طلاب متميزين لكل شعبة', 'error');
            return;
        }
        ...
        showMessage('تمت إضافة الطالب إلى قائمة المتميزين بنجاح', 'success');
        manageDistinguishedStudents(branchId);
    } catch (error) {
...
async function removeDistinguishedStatus(studentId, branchId) {
    ...
    try {
        ...
        showMessage('تمت إزالة الطالب من قائمة المتميزين بنجاح', 'success');
        manageDistinguishedStudents(branchId);
    } catch (error) {
